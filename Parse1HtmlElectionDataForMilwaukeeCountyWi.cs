...
namespace Parse1HtmlElectionDataForMilwaukeeCountyWI
{
	...
	class Program
	{
		static string ProjectName = "Parse1HtmlElectionDataForMilwaukeeCountyWI";

		static string LocationBeingStudied = "MilwaukeeCountyWI";

		static string LocationBeingStudiedClearly = "Milwaukee County Wisconsin";

		static string gitHubRepositoryURL = "https://github.com/IneffablePerformativity/" + ProjectName;

		static string gitHubRepositoryShortened = "https://bit.ly/3lD1Cxj";
...
//		static int howOrdering = 7; // 7 is ballots
		...
		const string GrainTag = "Ward"; // XML tag name e.g., Precinct, County, Ward, etc.
		...
		static string inputHtmlFileName = "Milwaukee County - 11-3-20 General and Presidential Election - Unofficial Results.html"; // sitting there
		...
		const string contestPotus = "President Vice President";

		
		// zero, one, or two
		const string contestSenus1 = "U.S. Senate"; // none here, only 'State Senator...'
		...
		const string contestRepus1 = "Representative in Congress, District 1";
		const string contestRepus2 = "Representative in Congress, District 4";
		const string contestRepus3 = "Representative in Congress, District 5";
		const string contestRepus4 = "Representative in Congress, District 6";
...
//
		...
		// re-adding the HTML input variables and method here at the bottom of this file:
		...
		static bool doDumpAllTablesFirst = false;
		...
		// note that although some strings are in this input routine,
		// they must also be added to the const strings for analysis.
		
		
		static void InputDataFromSavedHtmlWebpage()
		{
			
			// This 1st geneneration input code is being revised
			// not to create ad hoc data structures, but to call:
			// ponderInput(contest, choice, party, grain, votes);
			
			
			string inputHtmlFilePath = Path.Combine(inputDir, inputHtmlFileName);
			HtmlAgilityPack.HtmlDocument doc = new HtmlAgilityPack.HtmlDocument();
			doc.Load(inputHtmlFilePath, Encoding.UTF8); // Firefox saved as UTF8 w/o BOM
			...
			if (tableCollection != null)
			{
				...
				for(int tableIndex = 3; tableIndex <= 71; tableIndex += 2)
				{
					...
					for(int j = 1; j <= nWardRows; j++)
					{
						...
						// Needed in 7th gen code:
						string wardName = wardRowCells[1].InnerText.Trim(caSpaceHighSpace);
						...
						for(int i = 1; i <= thisRace.QtyCandidates; i++)
						{
							...
							int intVotes = int.Parse(wardRowCells[skip2Columns].InnerText.Trim(caSpaceHighSpace));
							thisRace.Candidates[i].WardsVotes.Add(wardNumber, intVotes);
							
							
							// this looks like the place to call ponderInput:
							
							// but only for federal contests
							if(thisRace.isPOTUS || thisRace.isREPUS)
							{
								string contest = thisRace.RaceName;
								string choice = thisRace.Candidates[i].CandidateName;
								string party = "";
								if(thisRace.Candidates[i].isDemocrat)
									party = "DEM";
								else if(thisRace.Candidates[i].isRepublican)
									party = "REP";
								else
									party = "ETC";
								string grain = wardName;
								string votes = wardRowCells[skip2Columns].InnerText.Trim(caSpaceHighSpace);
